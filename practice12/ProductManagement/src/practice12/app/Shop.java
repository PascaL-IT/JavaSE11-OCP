...
import practice12.data.Food;
import practice12.data.Product;
import practice12.data.ProductManager;
import practice12.data.ProductManagerException;
import practice12.data.Rateable;
import practice12.data.Rating;
import practice12.data.Review;
...
public class Shop {
	...
	public static void main(String[] args) {
		...
		Product p12;
		try {
			p12 = pm.findProduct(105);
			System.out.println("p12: " + p12);
		} catch (ProductManagerException e) {
			System.err.format("\nProductManager exception : %s", e.getMessage());
			// e.printStackTrace();
		}
		...
		pm.printProducts(cp1.thenComparing(cp2).thenComparing(cp3));
		...
		pm.printProducts(c5, p5);

		// Test get product discounts
		Map<String, String> discounts = pm.getDiscounts();
		discounts.forEach((rating, discount) -> {
			System.out.format("%s %s \n", rating, discount);
		});

		// Test various circumstances on which exceptions will be thrown...
//		pm.printProductReport(42);
//		pm.reviewProduct(42, Rating.NOT_RATED, "Mon avis sur le produit n°42");
//		try {
//			pm.findProduct(42);
//		} catch (ProductManagerException e) {
//			e.printStackTrace();
//		} 
		...
		// Test parse review (new method)
		try {
//			pm.parseReview("42,1, ...");
//			pm.parseReview("X,1, ...");
			pm.parseReview("101,3,OK (3 étoiles)");
			pm.parseReview("101,4,GOOD (4 étoiles)");
			pm.parseReview("101,5,VERY GOOD (5 étoiles)");
//			pm.parseReview("101,6,VERY GOOD (5 étoiles)");
			pm.printProductReport(101);
		} catch (ProductManagerException e) {
			e.printStackTrace();
		}

		// Test parse product (new method)
		try {
			//pm.parseProduct("X,42,3,OK,,(3 étoiles)"); // => NumberFormatException
			//pm.parseProduct("X,42,3,5.37,2,(3 étoiles)"); // => ProductManagerException
			pm.parseProduct("D,42,Coca-Cola,5.37,2,(3 étoiles)"); // => OK
			// pm.parseProduct("F,42,Coca-Cola,5.37,3"); // => DateTimeParseException
			pm.parseProduct("F,43,Fish&Ships,6.85,4,2021-04-18"); // => DateTimeParseException
			pm.printProductReport(42);
			pm.printProductReport(43);
		} catch (ProductManagerException e) {
			e.printStackTrace();
		}
